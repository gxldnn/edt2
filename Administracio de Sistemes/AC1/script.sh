...
function comprovar_logica(){
    ...
    if [ -f "$input" ] || [ -d "$input2" ] ; then
        ...
        echo "El directori \"$input2\" no existeix!!"
    elif [ ! -f "$input" ] || [ -d "$input2" ] ; then
        echo "El arcxiu \"$input\" no existeix!!"
    else
...
while true; do
    clear
    menu
    echo "" # Espacio en blanco entre en menu y la pregunta
    read -p "Que vols executar?: " eleccio
    clear # clear despues de la eleccion
    while true; do
        case "$eleccio" in
            1)
                clear
